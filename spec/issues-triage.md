...
| # | Title | Plan |
| --- | --- | --- |
| 189 | `isSubtypeOf` in scanner swallows all `Throwable` | **Fixed (2026-08-30, PR #227).** Narrowed the catch to `LinkageError` (the only error `isAssignableFrom` can throw, mirroring `loadClass`'s PR #188 narrowing); other errors now propagate, and a skipped class is reported to stderr. Deliberately no synthetic linkage-failure test: the scanner is currently dormant (legacy `Jarmonica*Main` flow, not wired to the published plugin), and a non-swallowed `LinkageError` path isn't reachable without a fabricated `ClassLoader`; happy path is covered by `PluginFlowTest`. |
